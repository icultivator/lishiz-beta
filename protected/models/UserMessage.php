...
class UserMessage extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, send_time', 'length', 'max'=>10),
			array('subject', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, subject, content, status, send_time', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function beforeSave(){
        if($this->isNewRecord){
            $this->send_time = time();
            $this->status = self::MSG_NOT_READ;
        }
        return parent::beforeSave();
    }

    public function send($user_id,$subject,$content){
