...
class Book extends CActiveRecord
{
    ...
    public function __construct($scenario='insert'){
        $this->oldTags = '';
        $this->obj_type = Yii::app()->params['obj_type']['book'];
	...
	public function rules()
	{
		...
		return array(
			array('title, author,content, cover', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('title, cover, tags', 'length', 'max'=>100),
            array('url', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, content, cover, tags, votes, views, user_id, status, create_time, last_update', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function message(){
        if(Yii::app()->user->id != $this->user_id) {
            ...
            $content = Yii::app()->user->name . Yii::t('opt', $this->getScenario()) . '您的' .
                Yii::t('item', ObjType::get($this->obj_type)) . '：' . CHtml::link($this->title, '/book/' . $this->id);
