...
class CollectController extends Controller
{
    ...
    public function actionAdd(){
        ...
        if($model->save()){
            echo CJSON::encode(array('success'=>'收藏成功！'));
        }else{
            echo CJSON::encode(array('msg'=>'收藏失败，请重试！'));
        }
    }
    ...
    public function actionCancel(){
        ...
        if(!$model){
            echo CJSON::encode(array('msg'=>'尚未收藏！'));
            exit;
        }
        if($model->delete()){
            echo CJSON::encode(array('success'=>'取消收藏成功！'));
        }else{
    ...
    private  function loadModel($obj_id,$obj_type,$user_id,$opt_type='add'){
        ...
        $model = UserCollect::model()->find('obj_id=:oid AND obj_type=:type AND user_id=:uid',
            array(':oid'=>$obj_id,':type'=>$obj_type,':uid'=>$user_id));
        switch($opt_type){
            case 'add':
                if($model){
                    echo CJSON::encode(array('msg'=>'已经收藏过了！'));
                    exit;
                }else{
